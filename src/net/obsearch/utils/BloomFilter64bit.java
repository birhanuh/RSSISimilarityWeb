...
public class BloomFilter64bit implements Serializable {
    ...
    private static Logger logger = Logger.getLogger(BloomFilter64bit.class);
     ...
     */
    private final ByteBuffer converter = ByteConversion.createByteBuffer(ByteConstants.Long.getSize());
    ...
    public BloomFilter64bit( final int n, final int d ) {
        ...
        logger.debug("Length for: n: " + n + " d: " + d + " len: " + len);
    ...
    private long hash(byte[] s, final int l, final int k ) {
        final long[] w = weight[ k ];
        long h = 0;
        int i = l;
        while( i-- != 0 ) h ^= s[ i] * w[ i % NUMBER_OF_WEIGHTS ];
    ...
    public boolean contains( long s ) {
    	...
    	byte[] sv = convertLong(s);
        int i = d, l = sv.length;
        while( i-- != 0 ) if ( ! getBit( hash( sv, l, i ) ) ) return false;
        return true;
    }
    
    private byte[] convertLong(long k){
