package net.obsearch.index.ghs.impl;
  ...
  import net.obsearch.index.CommonsLong;
  ...
  import net.obsearch.index.IndexLong;
  import net.obsearch.index.bucket.impl.BucketObjectLong;
  import net.obsearch.index.bucket.sleek.SleekBucketLong;
  ...
  import net.obsearch.result.OBResultInvertedLong;
  import net.obsearch.result.OBResultLong;
  import net.obsearch.index.ghs.SketchProjection;
  
  public final class Sketch64Long<O extends OBLong> extends AbstractSketch64<O, BucketObjectLong<O>, OBQueryLong<O>, SleekBucketLong<O>>
  implements IndexLong<O> {
  	
  	private boolean DEBUG = true;
  	
  	private static final transient Logger logger = Logger
  	.getLogger(Sketch64Long.class.getName());
  	 ...
  	 * @throws IOException
  	 */
  	public Sketch64Long(Class<O> type,
  	...
  	@Override
  	public BucketObjectLong<O> getBucket(O object) throws OBException,
  			InstantiationException, IllegalAccessException {
  		BucketObjectLong<O> res = new BucketObjectLong<O>(null, -1L, object);
  		return res;
  	}
  	...
  	public SketchProjection getProjection(BucketObjectLong<O> bucket) throws OBException {
  		...
  		while(i < m){
  				long distanceA = pivotGrid[i][0].distance(bucket.getObject());
  				long distanceB = pivotGrid[i][1].distance(bucket.getObject());
 	...
 	@Override
 	protected SleekBucketLong<O> instantiateBucketContainer(
 			byte[] data, byte[] address) throws InstantiationException, IllegalAccessException, OBException {
 		if(data == null){
 			return new SleekBucketLong<O>( type, bucketPivotCount);
 		}else{
 			try{
 				return new SleekBucketLong<O>( type, bucketPivotCount, data);
 			}catch(IOException e){
 	...
 	protected int primitiveDataTypeSize() {
 		return ByteConstants.Long.getSize();
 	}
 	...
 	@Override
 	public void searchOB(O object, long r, OBPriorityQueueLong<O> result)
 	...
 	protected AbstractOBQuery<O> getKQuery(O object, int k) throws OBException, InstantiationException, IllegalAccessException {
 		BucketObjectLong<O> b = getBucket(object);
 		OBPriorityQueueLong<O> result = new OBPriorityQueueLong<O>(k);
 		OBQueryLong<O> q = new OBQueryLong<O>(object, Long.MAX_VALUE, result, b
 				.getSmapVector());
 		return q;
 	}
 	...
 	@Override
 	public void searchOB(O object, long r, Filter<O> filter,
 			OBPriorityQueueLong<O> result) throws NotFrozenException,
 			InstantiationException, IllegalIdException, IllegalAccessException,
 			OutOfRangeException, OBException {
 		BucketObjectLong<O> b = getBucket(object);
 
 		OBQueryLong<O> q = new OBQueryLong<O>(object, r, result, b
 				.getSmapVector());
 		...
 		for(SketchProjection bucket: sortedBuckets){
 				SleekBucketLong<O> container = this.bucketCache.get(bucket.getAddress());
         //SleekBucketLong<O> container = this.instantiateBucketContainer(this.Buckets.getValue(bucket.getAddress()), bucket.getAddress());
 	...
 	public long[] fullMatchLite(O query, boolean filterSame) throws OBException, IllegalAccessException, InstantiationException{
 			return CommonsLong.fullMatchLite((OBLong)query, filterSame, this);
 	}
 	...
 	protected void maxKEstimationAux(O object)
 		...
 		long [] sortedList = fullMatchLite( object, true);
 		//`calculate the AVG distance between the sample and the DB.
 		for(long t : sortedList){
 		...
 		//List<OBResultLong<O>> sortedBuckets2 = fullMatch(object);
 		...
 		while (i < getMaxK().length) {
 			...
 			OBQueryLong<O> query = (OBQueryLong<O>) queryAbst;
 			
 			for (SketchProjection result : sortedBuckets) {
 				
 					SleekBucketLong<O> container = this.bucketCache.get(result.getAddress());
 					//SleekBucketLong<O> container = this.instantiateBucketContainer(this.Buckets.getValue(result.getAddress()), result.getAddress());
 				// search the objects
 				...
 				if (ep <= this.getExpectedEP() && query.isFull()) {
 					...
 					long difference = (long)Math.abs(sortedList[0] - query.getResult().getSortedElements().get(0).getDistance());
 					break; // we are done.
 				}
 			}
 			i++;
